...
Historical regular-Chat Sol Pro delegation has been verified: an inspected `gpt-5-6-pro` response contained native Subagent activity events, a roster of six completed agents and separately accessible reports. A negative smoke test therefore does not establish that Pro Chat cannot use hosted agents. Current availability must be verified for the actual run; fresh chats, a normal-Chat branch and a continuation of that historical chat did not produce new agents in the September 9 experiments.
