...
REPOSITORY="${CODEX_WEB_GPT_REPOSITORY:-mikkel32/codex-web-gpt-enhanced}"
...
VERSION="${VERSION#v}"
if [ -z "$VERSION" ]; then
  echo "Could not resolve the latest Maria WebGPT release" >&2
  exit 1
fi
case "$VERSION" in
...
if [ "$OS" = "Darwin" ]; then
  ...
  SOURCE_APP="$STAGE_DIR/Maria WebGPT.app"
  if [ ! -d "$SOURCE_APP" ] || [ ! -x "$SOURCE_APP/Contents/MacOS/Maria WebGPT" ]; then
  ...
  TARGET_APP="$INSTALL_DIR/Maria WebGPT.app"
  if pgrep -x "Maria WebGPT" >/dev/null 2>&1; then
    echo "Quit Maria WebGPT before updating it" >&2
    exit 1
  fi
  BACKUP_APP="$TEMP_DIR/Maria WebGPT.previous.app"
...
TARGET="$TARGET_DIR/Maria WebGPT.AppImage"
...
if { [ -n "$RUNNING_PID" ] && kill -0 "$RUNNING_PID" 2>/dev/null; } \
  || pgrep -f "Maria WebGPT\\.AppImage" >/dev/null 2>&1; then
  echo "Quit Maria WebGPT before updating it" >&2
  exit 1
fi
