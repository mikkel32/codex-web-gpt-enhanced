...
function installErrorReporting({ app, launcherWindow, coreHome, getBrowserHost = () => null, electron = require("electron"), senderFactory, now = Date.now }) {
  ...
  const snapshot = () => {
    ...
    const deliveryReady = settings.enabled && Boolean(account);
    return { ...settings, sender: account || settings.recipient, senderConfigured: Boolean(account), deliveryReady, lastFailure,
      deliveryStatus: !settings.enabled ? "Reporting is disabled."
        : !account ? "Recording and queueing locally. Set up the Gmail sender to deliver emails."
      ...
      reports: records.sort((a, b) => b.createdAt - a.createdAt).map(report => ({ id: report.id, createdAt: report.createdAt,
        source: report.source, state: report.delivery.state, reason: report.delivery.reason || "Waiting to send",
  ...
  const handler = async (event, request) => {
    ...
    if (request.action === "save") {
      ...
      if (value.password) sender.save(value.sender.trim(), value.password);
      else if (value.sender && sender.configured() && value.sender.trim() !== sender.account()) throw new Error("A new sender requires its Google app password");
      ...
      store.configure({ enabled: value.enabled, includeResponses: value.includeResponses, recipient: value.recipient });
      // A credential correction is explicit permission to retry a known rejected email.
      if (value.password) for (const report of store.records()) if (report.delivery.state === "blocked" && report.consentId === previous.consentId) {
        store.update(report, { state: "pending", attempts: 0, nextAttemptAt: Date.now(), reason: "Sender credentials updated" });
      }
      lastFailure = "";
    ...
    if (request.action === "test") {
      if (!store.settings().enabled || !sender.configured()) throw new Error("Save and enable report delivery first");
