...
function fixture(options){
 ...
 window.codexWebLauncher=new Proxy({snapshot:async()=>snapshot,doctor:async()=>({ok:true,checks:[]}),logs:async()=>[],connectionStatus:async()=>{window.testCalls++;return {nativeAvailable:true,browserConnected:true,activeBrowserTurns:0,recoveryAvailable:true};},setBrowserSurfaceActive:async()=>b,setBrowserBounds:async()=>true,copyNativeCodexCommand:async()=>true}, {get(t,k){if(k in t)return t[k];if(String(k).startsWith('on'))return listener=>{(listeners[k]??=new Set()).add(listener);return()=>listeners[k].delete(listener);};return async()=>state;}});
}
(async()=>{const browser=await chromium.launch({...(process.env.MARIA_CHROMIUM_PATH?{executablePath:process.env.MARIA_CHROMIUM_PATH}:{channel:'chromium'}),headless:true});const results=[];
try{for(const [name,root] of [...(process.env.MARIA_BASELINE_RENDERER ? [['before',path.resolve(process.env.MARIA_BASELINE_RENDERER)]] : []), ['after',path.resolve(__dirname,'../dist')]]){
 ...
 try{for(const width of name==='after'?[1180,700]:[1180]){
  const context=await browser.newContext({viewport:{width,height:1000}});await context.addInitScript(fixture,{biggerContext:name==='before'});const page=await context.newPage();const errors=[];page.setDefaultTimeout(10000);page.on('pageerror',e=>errors.push(e.message));
  console.log('PERFORMANCE_CASE',name,width);await page.goto(url,{waitUntil:'domcontentloaded',timeout:10000});await page.getByRole('button',{name:'Open ChatGPT',exact:true}).waitFor();await page.waitForTimeout(900);
  ...
  if(name==='after') {
   ...
   await page.getByRole('button',{name:'Overview',exact:true}).click();await page.locator('.guided-home').waitFor({state:'visible'});
