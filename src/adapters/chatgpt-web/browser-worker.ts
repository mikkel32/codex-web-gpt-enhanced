...
export async function throwIfChatGptSessionFailureAlert(page: Page): Promise<void> {
  if (await chatGptExpiredSessionAlert(page).isVisible().catch(() => false)) {
    throw new ChatGptWebAdapterError(
      "The ChatGPT session has expired. Sign in again in Maria WebGPT.",
...
export function assertChatGptWebInputWithinLimits(
  ...
  if (modelId !== CHATGPT_WEB_MODEL_ID && modelId !== CHATGPT_WEB_ASTRA_MODEL_ID && modelId !== CHATGPT_WEB_LUNA_MODEL_ID) {
  ...
  if (
    ...
    throw new ChatGptWebAdapterError(
      `This prompt contains ${promptChars.toLocaleString("en-US")} inline characters, which exceeds the measured ${browserComposerCharLimit.toLocaleString("en-US")}-character ChatGPT composer boundary for this account and effort. Reduce the visible message or use context-file delivery before retrying.`,
      { status: 400, errorType: "invalid_request_error", code: "chatgpt_composer_size_exceeded", retryable: false },
    );
  }
...
export class ChatGptBrowserWorker {
  ...
  private async prepareTemporaryChatSurface(
    ...
    const startUrl = savedConversation ? "https://chatgpt.com/" : CHATGPT_TEMPORARY_CHAT_URL;
    ...
    if (!savedConversation) await assertTemporaryChatPage(page);
  ...
  private async runExclusive(turn: BrowserTurn): Promise<string> {
    ...
    try {
      ...
      try {
        const checkpointAccepted = terminal === "aborted" && turn.nativeConnector && turn.requireRetainedConversation
  ...
  private async runBrowserTurn(
    ...
    try {
      ...
      // File transport changes only the browser envelope, never the configured context capacity.
      assertChatGptWebInputWithinLimits(estimatedInputTokens, estimatedMessageTokens, turn.modelId,
        requestedMode.effort, browserCapabilities, maxMessageChars);
      chatGptPromptFilePayloads(prepared); // Validate every attachment before mutating the composer.
      const stagingMode = requestedMode;
      ...
      const finalSubmissionEvidence = await this.runStage(
        turn.traceId,
        "send",
        // Large file attachments need additional ingestion headroom after the single Send.
