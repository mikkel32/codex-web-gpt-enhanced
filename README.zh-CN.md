...
已验证可用的 `5.15.0-alpha.10` 实现现升级为 **5.20.0**，移除 alpha 标记。
维护者报告，工作流程在其当前 macOS 环境中已正常运行，没有发现问题。

本次发布保留了带确认回执的上下文传递、会话连续性、由 ChatGPT 管理的 Web 上下文、
原生连接验证，以及插件执行结果和模型选择方面的改进。
具体变更和验证范围请参阅 [5.20.0 发布说明](docs/release-5.20.0.md)。
