...
## Maria 5.20: 大きな進歩を遂げた安定版

動作実績のある `5.15.0-alpha.10` の実装を、alpha 表記のない **5.20.0** として公開します。
メンテナーは、現在の macOS 環境でワークフローが問題なく動作していると報告しています。

確認応答付きのコンテキスト配信、会話の継続、ChatGPT による Web コンテキスト管理、
ネイティブ接続の検証、プラグインの実行結果とモデル選択の改善を引き継ぎます。
変更内容と検証範囲は [5.20.0 リリースノート](docs/release-5.20.0.md) を参照してください。
