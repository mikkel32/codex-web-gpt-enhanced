...
Context results contain one JSON text block, capped at 24 KiB after serialization. Pages contain at most 12,000 UTF-16 code units and preserve Unicode boundaries; always follow `next_offset` rather than assuming a fixed increment. The transport contract allows at most two retries of the identical read-only page after a timeout or temporary connection interruption. Authorization, safety, and invalid-offset errors remain terminal. Context diagnostics record the offset, result byte count, and local completion time without logging context contents.
