...
for (const scenario of ["success", "unacknowledged", "invented-proof", "browser-failure", "cancel"] as const) {
  test(`connection verification: ${scenario} retires only its own capability`, async () => {
    ...
    const read = async (name: string, offset: number, receipt?: string) => {
      const response = await client.callTool({ name: "codex_context_read", arguments: { turn_token: token, name, offset, ...(receipt ? { receipt } : {}) } });
    ...
    try {
      const outcome = verifyConnectionRoundTrip({ broker, cwd: tmpdir(), signal: controller.signal, run: async turn => {
        ...
        const search = await client.callTool({ name: "codex_context_search", arguments: { turn_token: token, query: "connection_evidence" } });
        ...
        const invoked = await client.callTool({ name: "codex_tool_call", arguments: { turn_token: token, wire_name: catalog.tools[0].wire_name, arguments: {} } });
      ...
      if (scenario === "success") expect(await outcome).toMatchObject({ ok: true, scope: ["required-context-receipts", "evidence-retrieval", "native-tool-round-trip"] });
      ...
      expect((await client.callTool({ name: "codex_context_read", arguments: { turn_token: token, name: "codex-context-1-of-2.json", offset: 0 } })).isError).toBe(true);
      expect(broker.beginCompletionFence(other)).toBeDefined();
    } finally { broker.revoke(other); await client.close(); await broker.close(); }
  }, 20_000);
}
